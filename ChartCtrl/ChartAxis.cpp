						  ...
						  double dTickVal)
{
	...
	double TempTickLabelValue = dTickVal;
	
	if (vVolumeSort)
	if (vValveLabels)
	if ((dTickVal <= vNumberOfValves) && ( dTickVal > 0))
		TempTickLabelValue = (double)vValveLabels[(int)dTickVal - 1];
	
	TChartString strBuffer = GetTickLabel(TempTickLabelValue);
